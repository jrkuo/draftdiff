...
def build_target_heroes_counter_heroes_df_for_dotabuff_id_in_last_n_days2(
    ...
    try:
        ...
        logger.info(f'[{io.get_io_location()}] Running making df from dfs for player {player_id} for last {n} days')
        df_hero_stats_for_player_id = io.read_df(f'dotabuff/player_stats-df/ds={ds}/player_id={player_id}/days={n}')
        dotabuff_counter_dfs: dict[str, pd.DataFrame] = {}
        stratz_counter_dfs: dict[str, pd.DataFrame] = {}
        for _, row in tqdm(
            ...
            dotabuff_counter_dfs[hero_name] = io.read_df(f'dotabuff/matchups-df/ds={ds}/hero={dotabuff_slug}')
            stratz_slug = stratz.get_stratz_slug_for_dota_hero(hero_name)
            stratz_counter_dfs[hero_name] = io.read_df(f'stratz/matchups-df/ds={ds}/hero={stratz_slug}')
...
def build_player_counters_df(
    df_hero_stats_for_player_id: pd.DataFrame,
    dotabuff_counter_dfs: dict[str, pd.DataFrame],
    stratz_counter_dfs: dict[str, pd.DataFrame],
):
    combined_dfs: list[pd.DataFrame] = []
    result_df: pd.DataFrame = pd.DataFrame()
    for _, row in df_hero_stats_for_player_id.iterrows():
        ...
        dotabuff_repeated_rows = pd.DataFrame([row] * df_dotabuff_counter_num_rows, columns=row.index)
        dotabuff_combined_df = pd.concat([dotabuff_repeated_rows.reset_index(drop=True), df_dotabuff_counter], axis=1)
        stratz_repeated_rows = pd.DataFrame([row] * df_stratz_counter_num_rows, columns=row.index)
        stratz_combined_df = pd.concat([stratz_repeated_rows.reset_index(drop=True), df_stratz_counter], axis=1)
...
def get_cached_weighted_avg_metrics_for_player_last_n_days(ds: str, player_id: str, n: int) -> pd.DataFrame:
    partition_path = f'output/player_counters_weighted-df/ds={ds}/player_id={player_id}/days={n}'
    try:
        player_weighted_counters_df = io.read_df(partition_path)
        logger.info(f'[{io.get_io_location()}] loaded cached df from {partition_path}')
    except Exception:
        logger.info(f'[{io.get_io_location()}] Running making df from df for player {player_id} for last {n} days')
        df_hero_counters_stats_for_player_id = io.read_df(
            f'output/player_counters-df/ds={ds}/player_id={player_id}/days={n}'
        )
        player_weighted_counters_df = calculate_weighted_avg_metrics_in_counter_heroes_df(
...
def calculate_weighted_avg_metrics_in_counter_heroes_df(
    ...
    grouped_df = counter_df.groupby(['hero', 'counter_hero'])  # type: ignore

    for (hero, counter_hero), df_group in grouped_df:  # type: ignore
        ...
        weighted_disadvantage = (
            df_group['target_disadvantage'] * df_group['counter_vs_target_matches_played']
        ).sum() / df_group['counter_vs_target_matches_played'].sum()
        weighted_win_percent = (
            df_group['target_winrate_vs_counter'] * df_group['counter_vs_target_matches_played']
        ).sum() / df_group['counter_vs_target_matches_played'].sum()
    ...
    output_df = pd.json_normalize(output_rows)  # type: ignore
    return output_df
...
def main2():
    ...
    for id in tqdm(player_id_list):
        build_target_heroes_counter_heroes_df_for_dotabuff_id_in_last_n_days2(ds=ds, player_id=id, n=n)
        get_cached_weighted_avg_metrics_for_player_last_n_days(ds=ds, player_id=id, n=n)


def main():
    ds = util.get_current_ds()
    hero_list: list[str] = list(constants.HERO_ID_DICT.keys())
    player_id_list = ['181567803']
    n = 30
    for hero_name in tqdm(hero_list):
        stratz.get_cached_matchup_stats(ds=ds, token=os.environ['STRATZ_API_TOKEN'], hero_name=hero_name)
    ...
    for id in tqdm(player_id_list):
        dotabuff.get_cached_dotabuff_match_pages_for_past_n_days(ds=ds, dotabuff_player_id=id, n=n)
        dotabuff.get_cached_heroes_stats_for_dotabuff_id_in_last_n_days(ds=ds, player_id=id, n=n)
        build_target_heroes_counter_heroes_df_for_dotabuff_id_in_last_n_days2(ds=ds, player_id=id, n=n)
        get_cached_weighted_avg_metrics_for_player_last_n_days(ds=ds, player_id=id, n=n)
