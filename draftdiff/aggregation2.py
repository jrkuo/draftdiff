...
def build_counter_heroes_df(ds) -> pd.DataFrame:
    ...
    try:
        ...
        for hero in tqdm(hero_list):
            ...
            dotabuff_counter_dfs[hero] = io.read_df(f'dotabuff/matchups-df/ds={ds}/hero={dotabuff_slug}')
            stratz_slug = stratz.get_stratz_slug_for_dota_hero(hero)
            stratz_counter_dfs[hero] = io.read_df(f'stratz/matchups-df/ds={ds}/hero={stratz_slug}')
...
def build_counter_df(dotabuff_counter_dfs, stratz_counter_dfs):
    ...
    for hero in tqdm(hero_list):
        ...
        db_stratz_df = pd.concat([dotabuff_counter_df, stratz_counter_df], axis=0, ignore_index=True)
        ...
        for counter_hero, df_group in grouped_df:
            ...
            total_head_to_head_matches = df_group['counter_vs_target_matches_played'].sum()
