...
def get_dotabuff_match_page_text_for_player_id(dotabuff_player_id, match_page_num) -> str:
    # Avoid rate limiting
    headers = {'User-Agent': 'Mozilla/5.0'}
    logger.warning(f'downloading dotabuff page {match_page_num} data for player {dotabuff_player_id}')
...
def get_cached_dotabuff_match_pages_for_past_n_days(ds: str, dotabuff_player_id: str, n: int):
    match_page_num = 1
    ...
    while earliest_difference_in_days <= n:
        ...
        try:
            ...
            match_page_text = get_dotabuff_match_page_text_for_player_id(dotabuff_player_id, match_page_num)
...
def get_cached_heroes_stats_for_dotabuff_id_in_last_n_days(ds: str, player_id: str, n: int) -> pd.DataFrame:
    ...
    try:
        ...
        logger.info(f'[{io.get_io_location()}] Running making df from html for player {player_id} for last {n} days')
        ...
        for file_path in file_paths:
            ...
            match_page_df = build_dotabuff_player_stats_from_match_page_text(match_page_data)
...
def get_dotabuff_slug_for_dota_hero(hero_name: str) -> str:
...
def get_cached_counters_page(ds: str, hero_name: str) -> str:
    dotabuff_hero_slug = get_dotabuff_slug_for_dota_hero(hero_name)
...
def test2():
    os.environ['IO_LOCATION'] = 'local'
    get_cached_heroes_stats_for_dotabuff_id_in_last_n_days(ds=util.get_current_ds(), player_id='181567803', n=30)
...
def test3():
    os.environ['IO_LOCATION'] = 'local'
    get_cached_dotabuff_match_pages_for_past_n_days(ds=util.get_current_ds(), dotabuff_player_id='181567803', n=30)
...
def build_dotabuff_counter_stats_from_hero_page_text(html_text) -> pd.DataFrame:
    ...
    for i, row in enumerate(tr[1:]):
        ...
        counter_vs_target_matches_played = int(counter_target_matches_played['data-value'])
        new_records += [
            {
...
def get_cached_hero_counters_for_hero(ds: str, hero_name: str) -> pd.DataFrame:
    dotabuff_hero_slug = get_dotabuff_slug_for_dota_hero(hero_name)
    ...
    try:
        ...
        logger.info(f'[{io.get_io_location()}] Running making df from html for {hero_name}')
        match_page_data = io.read_html(f'dotabuff/matchups/ds={ds}/hero={dotabuff_hero_slug}')
        match_page_df = build_dotabuff_counter_stats_from_hero_page_text(match_page_data)
...
def main():
    ...
    for id in tqdm(player_id_list):
        get_cached_dotabuff_match_pages_for_past_n_days(ds=ds, dotabuff_player_id=id, n=n)
